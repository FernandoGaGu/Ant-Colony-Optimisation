...
seed = 1997
n_jobs = 8
pheromone_update = {'strategy': 'as', 'weight': 0.15, 'graph_type': graph_type}
accessory_node = True
Q = 0.1
R = 0.05
...
local_search = hybrid.greedy.GreedySearchOP(add_to_old=True,
...
antco.graphics.branchingFactor(report, save_plot='./convergence/sd_hybrid_AS(more_edges)_BF.png')
plt.show()
antco.graphics.convergence(
    report, title='\nSolution %.4f (Optimal %.4f)\n' % (solution_score, optimal_solution_score),
    save_plot='./convergence/sd_hybrid_AS(more_edges)_convergence.png')
plt.show()
