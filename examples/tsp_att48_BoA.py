...
colony = antco.ACO(
    ...
    iterations=iterations, graph_type=graph_type, evaporation=evaporation,
    alpha=alpha, beta=beta, pheromone_init=pheromone_init, path_limits=(n_nodes-1, n_nodes),
...
start = time.time()
report = antco.algorithm.bagOfAnts(colony, bag_size=elite_size, out_of_bag_size=elite_size//2)
end = time.time()
...
antco.graphics.branchingFactor(report, save_plot='./convergence/att48_BoA_BF.png')
plt.show()
