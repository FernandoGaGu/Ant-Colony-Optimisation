...
def generate_problem(nodes: int, edges: int, optimal_path_length: int, noise: int,
    ...
    while not valid_subgraph:
        ...
        count += 1
        if count == saveguard:
            assert False, 'impossible to obtain a connected subgraph from the network, consider increasing the number of connections.'
    ...
    for _ in range(noise):
        noise_nodes = np.random.choice(
            [n for n in range(nodes)], size=random.randint(min_noise_length, max_noise_length), replace=False)

        # To control that introducing noise does not create sub-networks with values greater than the 
        # optimal sub-network
        repeated = False
        ...
        if not add_more:
            print('Too much noise has been introduced, the added noise may generate a better than optimal sub-network.')
            break
        ...
        variation = np.multiply(np.random.uniform(low=0.2, high=0.35, size=adj_matrix.shape),
                                random_subnetwork)
        Gmax += variation
        Gmin -= variation
